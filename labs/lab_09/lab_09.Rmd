...
**A1.** 
I chose this chart because it visualizes the results of each election and is organized by county.

https://www.datawrapper.de/_/Kr8dC/ 
...
md_gov_county22_18 <- md_gov_county %>% left_join(md_gov_county_18, by="GEOID", "County")

md_gov_county22_18 %>% 
  mutate(total_votes = Cox + Hogan) %>% 
  mutate(pctcox = Cox/total_votes *100) %>% 
  mutate(pcthogan = Hogan/total_votes *100)

write_csv(md_gov_county22_18, "data/md_gov_county22_18.csv")
```
...
**A2.** Maybe a story on why total votes were so drastically different in 2020 compared to the 2018 and 2022 total votes.
https://www.datawrapper.de/_/0dJzv/

```{r}
md_votes_by_type <- read_csv("data/md_votes_by_type.csv") %>% clean_names()

md_votes_by_type <- md_votes_by_type %>%
  mutate(total_votes = total) %>% 
  mutate(pctearly = early/total_votes *100) %>% 
  mutate(pctdayof = election_day/total_votes *100)
