...
**Answer** The loans in states and the candidates they voted for, it gives us the percentage of loans for small businesses and the correlation for the candidate they voted for, and their loans. 
```{r}
...
**Answer** The more businesses that supported Trump correlates with the loan amounts given to the small business. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** This code shows us the coorelation for the ppe amount per employee and the coorelation to them voting for Trump. The relationship is not as close as the last one. 
```{r}
...
**Answer** The table shows us the columns in the dataframe that are numeric, and runs the correlating function as well as the correlation coefficient. The table shows us the small businesses receiving lones and the correlation coefficient to go with each. The .52 coefficient is mildly positive (right in the middle) and .22 is closer to 0 and weak positive relationship. 
...
**Answer** This model gives us the correlation coefficient at the bottom. The top gives us teh p-value, which is low, meaning that there is a little chance that the .52 is random. We can be confident that this is a real relationship.  
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** This is the same as the above accept this model is for the employee, giving us a lower p-value, which we shouldn't trust as much as the above one. 
```{r}
...
**Answer** This table shows us the average for employees and for small businesses getting loans in the states where Clinton won and the statets where Trump won. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** The t-test shows us the amounts per employee, showing us the t = -1.2223, which we know is the difference in means in the two groups. The p-value is pretty high, and there should not show the confidence that this is a meaningful relationship. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** This is for businesses receiving loans, the t-test is stronger as it gets larger (both positive and negative) and in this case it is getting larger. The p-value is smaller so we can be more confident about trusting the results and producing a story. 
```{r}
