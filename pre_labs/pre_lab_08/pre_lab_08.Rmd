...
**Answer** The name of the data frame is changed to calls by month, and sorting the data by month and total calls in those months. 
...
**Answer** I see a grey square, probably where data should go. A blank canvas for information.
...
**Answer** I now see a bar chart that represents the data in the data frame.
...
**Answer** This bar chart shows the data, and it is gradually increasing as the months go on. The chart is now reoordered in order from least to most by month. 
...
**Answer** This chart is now flipped on the side and shows the data as if it is a race to the finish line. 
...
**Answer** The chart is still horizontal but now without a grey scale background on each line, it is more clean looking to me.
...
**Answer** This chart now has a blue background underneith the bar chart, the bars are now white and not black.
```{r}
baltco_911_calls_by_month %>%
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Answer** There is now a title at the top of the chart, showing what the data represents if you didn't know from the data frames. 
...
**Answer** There are dates on the left hand side and next to each date is the total number of calls on that specific day.
...
**Answer** This chart is more of a wavelength, showing the data in a dramatic and skewed way. It is hard to read and decipher where the data is. 
...
**Answer** The chart itself is the same, but the date column is all jambled together, yoou can't tell what is is showing. 
...
**Answer**I can now read the date for each line, but it is still hard to read each oone without focusing on each date.
